...
It has its own server, dependencies, storage keys, and UI. It does not use the
parent GropBox application, Supabase, Google Drive, or its environment files.
...
not covered by the parent repository's MIT license. The downloaded responses did
not include a separate artwork license.
