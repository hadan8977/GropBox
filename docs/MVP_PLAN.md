...
Image attachments lazy-load Drive thumbnails through an authenticated, owner-checked endpoint (raster images only, capped at 2 MiB). Thumbnail URLs and Google credentials stay server-side. A per-account memory cache holds up to 8 MiB for five minutes; it is not an offline image archive. If Drive has not generated a thumbnail, a browser-readable original up to 2 MiB can fill in. Other unavailable previews offer Retry without blocking downloads or marking message sync offline. Opening a preview loads the original only for supported images up to 12 MiB; larger or unsupported originals keep the thumbnail. File-type icons use MIME types and filename extensions, including in the transfer queue, not OS-installed application icons.
...
Material depth comes from low-contrast color, not bright perimeter strokes. Search and composer use a single inset focus outline; toolbar and mobile navigation have no surrounding glass shell. The send control is flat and loses its filled background when disabled. High-contrast preferences restore stronger borders and focus indicators; reading panels remain opaque.
